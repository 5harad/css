...
# 2.
# Note that urlopen is imported here to keep the scripts relevant to problem 2
...
class OnlineBookReader(BookReader):
    ...
    def __init__(self, URL, filename='tmp'):  # set a default filename
        self.link = urlopen(URL)  # open the url
        text = self.link.read()
        with open(filename, 'wb') as f:  # open file to write on
            f.write(text)
