...
- For most cases, there will be a much faster, *vectorized* alternative to 
looping
- We cover loops because there are some *rare* cases in which a loop might make 
more sense, but in general, loops should be avoided when writing `R`

[^loops]: Not necessarily true, but loops are still best avoided for other reasons too.
...
library("tidyverse")
```
- Note: If you ever need to use the two packages `plyr` and `tidyverse` within 
the same workspace (which you will), ALWAYS (!!!) load `plyr` first!
