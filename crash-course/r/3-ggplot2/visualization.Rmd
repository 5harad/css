...
- NOTE: The `CRAN` version of `ggmap` is currently broken, due to recent Google Maps
  policy changes, so we must install a "development" version via Github
```{r, eval=FALSE}
# Install, if you haven't already.
# Only need to do this once on a single machine.
# install.packages('ggmap')
...
- The main arguments for `qmap` are
    ...
    - `source`: Source to load maps from; `google` by default, but not
      recommended due to API requirements
...
- From the `crime` data, plot crimes reported on a Sunday during January, in
  downtown Houston, where the colors represent offense type (found in `offense`
  column)
- Use `zoom` level *14*
...
```
- What other wasy would you visualize this data on a map?
