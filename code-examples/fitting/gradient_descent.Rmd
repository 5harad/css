...
```{r animated_gganimate, echo=FALSE, message=FALSE, warning=FALSE, fig.show="animate"}
...
p <-  ggplot(example_data, aes(x=x, y=y)) +
  ...
  #   geom_raster(interpolate=TRUE, aes(fill=log(loss))) +
  #   geom_point(x=b, y=a, color='red', shape=4) +
  #   geom_point(data=coef_now, aes(x=b, y=a)) +
  #   geom_line(data=coef_cum, aes(x=b, y=a), linetype='dashed') +
  #   scale_fill_gradient(low='grey10', high='grey90', guide=FALSE) +
...
```{r animated_loss_gganimate, echo=FALSE, message=FALSE, warning=FALSE, fig.show="animate"}
xlb <- -1   #min(sample_coefs$b) - 0.2
xub <- 3    #max(sample_coefs$b) + 0.2
ylb <- -1   #min(sample_coefs$a) - 0.2
yub <- 3  #max(sample_coefs$a) + 0.2
...
loss <- ggplot(data=models, aes(x=b, y=a)) +
  geom_raster(interpolate=TRUE, aes(fill=log(loss))) +
  geom_point(x=b, y=a, color='red', shape=4) +
  geom_point(data=sample_coefs, aes(x=b, y=a, frame=iter)) +
  geom_line(data=sample_coefs, aes(x=b, y=a, cumulative=TRUE, frame=iter), linetype='dashed') +
  scale_fill_gradient(low='grey10', high='grey90', guide=FALSE) +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0))
